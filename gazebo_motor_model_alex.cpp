...
void GazeboMotorModel::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf) {
  ...
  //-------editado
  if (_sdf->HasElement("paletNumber"))
    palet_number_ = _sdf->GetElement("paletNumber")->Get<int>();
  else
    gzerr << "[gazebo_palet_model] Please specify a paletNumber.\n";
  ...
  getSdfParam<std::string>(_sdf, "tiltSubTopic", tilt_sub_topic_, tilt_sub_topic_); //editado
  ...
  */
  ...
  command_tilt_sub_ = node_handle_->Subscribe<mav_msgs::msgs::CommandMotorSpeed>("~/" + model_->GetName() + tilt_sub_topic_, &GazeboMotorModel::Velocity1Callback, this);
...
void GazeboMotorModel::Velocity1Callback(CommandMotorSpeedPtr &rot1_velocities) {
  if(rot1_velocities->motor_speed_size() < palet_number_) {
    std::cout  << "You tried to access index " << palet_number_
      << " of the MotorSpeed message array which is of size " << rot1_velocities->motor_speed_size() << "." << std::endl;
  } else ref_tilt_rot_vel_ = std::min(static_cast<double>(rot1_velocities->motor_speed(palet_number_)), static_cast<double>(max_rot_velocity_));
}
...
void GazeboMotorModel::UpdateForcesAndMoments() {
  ...
  double scalar = ref_tilt_rot_vel_; // at 25 m/s the rotor will not produce any force anymore
  scalar = ignition::math::clamp(scalar, -1.0, 1.0);
